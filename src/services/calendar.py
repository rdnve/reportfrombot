...
class ListEventService:
    ...
    def __call__(self) -> Tuple[List[any], List[any]]:
    ...
    def filter_data(self, d) -> List[any]:  # noqa: VNE001
        return list(filter(lambda x: x.begin.date() == d, self.cal_data.events))

    def get_sorted_events_by_date(self) -> Tuple[List[any], List[any]]:
    ...
    def get_events(self, events) -> List[any]:
        res = []
